...
The package contains two clients with very similar APIs (one offering an asynchronous interface based on `asyncio`, the other offering a blocking interface) to interact with a NatNet server (i.e., a Motive application). For example, using the blocking interface, a minimal program that connect to a discoverable server and then fetch a single MoCap update is:
